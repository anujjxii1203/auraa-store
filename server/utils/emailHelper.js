async function sendReturnEmail(toEmail, orderId, productName, reason) {
  ...
  let formattedReasonHTML = `<p style="color: #666;">Reason: <i>${reason}</i></p>`;
  try {
    ...
    formattedReasonHTML = `
      <div style="background: #fff; padding: 15px; border-radius: 5px; border: 1px solid #ddd; text-align: left; margin: 15px 0;">
        <p style="margin: 0 0 10px 0; color: #333;"><b>Return Type:</b> ${String(parsedReason.returnType || 'Refund').toUpperCase()}</p>
        <p style="margin: 0 0 10px 0; color: #333;"><b>Primary Reason:</b> ${parsedReason.primaryReason || 'N/A'}</p>
        ${parsedReason.details ? `<p style="margin: 0 0 10px 0; color: #333;"><b>Details:</b> ${parsedReason.details}</p>` : ''}
