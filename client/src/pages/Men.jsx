...
const Men = () => {
  ...
  return (
    <main>
      ...
      {/* Hero Section */}
      <section className="hero-full" style={settings?.men_hero_banner_image ? {
        backgroundImage: `url(${settings.men_hero_banner_image})`,
        ...
        <div style={{ position: 'relative', zIndex: 1, color: '#fff', padding: '2rem' }}>
          <h1 style={{ fontSize: '3.5rem', fontWeight: '950', marginBottom: '0.5rem', letterSpacing: '1px' }}>{settings?.men_hero_banner_text || "MEN'S STREETWEAR"}</h1>
          <p style={{ fontSize: '1.2rem', marginBottom: '1.5rem', fontWeight: '600' }}>{settings?.men_hero_subtext || "Explore premium tees, jackets, pants and more."}</p>
          {settings?.men_hero_button_text && settings?.men_hero_button_link && (
            <div style={{ textAlign: 'center', width: '100%' }}>
              <a href={settings.men_hero_button_link} style={{
